...
<template>
    <UContainer>
        ...
        <div class="mt-4" v-if="displayRoute">
            <UCarousel arrows dots v-slot="{ item }" :start-index="selectedRouteIndex" :items="routes" @select="(index) => {selectedRouteIndex=index; routeStore.activeRoute = routeStore.routes.at(index) }">
                <UForm>
